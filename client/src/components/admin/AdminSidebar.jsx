        ...
        {adminNavlinks.map((link) => (
          <NavLink
            ...
            className={({ isActive }) =>
              ...
              ${isActive ? 'bg-blue-500/15 text-blue-500' : 'hover:bg-gray-800'}`
            }
          >
            {({ isActive }) => (
              <>
                <link.icon className={`w-5 h-5 ${isActive ? 'text-blue-500' : 'text-gray-400'}`} />
                <p className="max-md:hidden">{link.name}</p>
                <span className={`w-1.5 h-10 rounded right-0 absolute ${isActive ? 'bg-blue-500' : ''}`} />
              </>
            )}
          </NavLink>
        ))}
