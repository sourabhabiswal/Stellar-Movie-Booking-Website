  ...
  return (
    <div className='px-6 md:px-24 xl:px-44 py-20 overflow-hidden'>
        ...
        <div className='relative mt-6'>
            ...
            <iframe src={currentTrailer.videoUrl} 
            width="960" height="560"
            className="mx-auto max-w-full rounded-xl block" allowFullScreen />
        </div>
        <div className='group grid grid-cols-4 md:gap-8 mt-8 max-w-3xl mx-auto'>
          {dummyTrailers.map((trailer, index) => (
            <div key={trailer.image} className='relative group-hover:not-hover:opacity-50 hover:-translate-y-1 duration-300 transition max-md:h-60 md:max-h-60 cursor pointer'  onClick={()=> setCurrentTrailer(trailer)}>
              ...
              <PlayCircleIcon strockeWidth={1.6} className='absolute top-1/2 left-1/2 w-5 md:w-8 h-5 transform translate-x-1/2 -translate-y-1/2 ' />
            </div>
          ))}
